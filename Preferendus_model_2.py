...
from Preferendus_model_settings_2 import settings
 
@xw.func
def white_box_run_model():
    """
    Function to run the categoric model optimalisation through Excel
    """ 
    ...
    def objective(variables):
       ...
       for i in range(categorical_num_objectives):
           formula = categorical_objective_formula_balance(i, categorical_objective_list, categorical_balance_sum_list, functions_null_scenario_balance_list, x1, x2, x3, x4, x5)
           active_p.append(formula)             
                   
       # aggregate preference scores and return this to the GA
       return stakeholder_weights_list, active_p
    ...
    ga = GeneticAlgorithm(objective=objective, constraints=cons, bounds=white_box_bounds_list, options=options)
    ...
    print(f'Optimal result for x1 = {round(design_variables_IMAP[0], 0)},  x2 = {round(design_variables_IMAP[1], 0)}, '
          ...
          f'x5 = {round(design_variables_IMAP[4], 0)}, (with sum = {round(sum(design_variables_IMAP))} houses)')

    # Fill in optimum values 
    preferendus_tool_ws.range(white_box_output_optimum).options(transpose=True).value = design_variables_IMAP
